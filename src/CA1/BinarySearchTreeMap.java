...
public class BinarySearchTreeMap<K extends Comparable<K>, V> {
    ...
    */
    ...
    public Set<K> keySet()
    {
        ...
        return keySetSub(root, new HashSet<K>()); // used a HashSet as Set is abstract and can't be instantiated
    }

    private Set<K> keySetSub(Node node, Set<K> key){
        key.add(node.key);
        if (node.left != null) key.addAll(keySetSub(node.left, key));
        if (node.right != null) key.addAll(keySetSub(node.right, key));
